...
import { format, startOfWeek, addDays, parseISO, startOfMonth, addMonths, subMonths, eachDayOfInterval, endOfMonth, isSameMonth, getDay } from 'date-fns';
  ...
  const handleReschedule = async () => {
    ...
    try {
      ...
      if (post?.platform == "instagram") {
        ...
        const scheduledResponse = await fetch(`${BACKEND_APIPATH.BASEURL}/schedule/post/api`, {
          method: "POST", headers: {
            'Content-Type': 'application/json',
    ...
    try {
      ...
      if (post?.platform == "instagram") {
        ...
        try {
          ...
          const accountInfo = await getSocialMediaAccountInfo("twitter");
          const { access_token, refresh_token } = accountInfo;
          const response = await fetch(`${BACKEND_APIPATH.BASEURL}/post/tweet/twitter`, {
            headers: {
...
export function Calendar() {
  ...
  const generateCalendarGrid = () => {
    ...
    const totalCells = 7 * Math.ceil((firstDayOfMonth + daysInMonth.length) / 7);
