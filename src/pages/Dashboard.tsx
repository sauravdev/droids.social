...
import { deleteSocialMediaAccount, getSocialAccounts, getSocialMediaAccountInfo, linkSocialAccount, updateSocialAccount, updateSocialAccountUsername } from "../lib/api";
...
export function Dashboard() {
  ...
  const [isOpenInstructionModal , setIsOpenInstructionModal] = useState<boolean>(false) ; 
  ...
  const findUsername = (platform : string   , accounts : any     ) => 
  {
  ...
  useEffect(() => {console.log("accountss = " , accounts )} , [accounts ]); 
  const fetchAndUpdateActiveSocialAccounts = async (apiKey : string ) => {
    ...
    try{
      ...
      console.log('active social accounts = '  ,  data?.data?.activeSocialAccounts); 
      let includedPlatforms = accounts.map((account : any  ) =>{ return account.platform}); 
      console.log("included platforms = " , includedPlatforms) ; 
      await Promise.all(
        includedPlatforms.map(async (platform: string) => {
          console.log("unlinking social media account =", platform);
          await deleteSocialMediaAccount(platform);
        })
      );
      includedPlatforms = [] ; 
      if(data?.data?.activeSocialAccounts && Array.isArray(data?.data?.activeSocialAccounts) && data?.data?.activeSocialAccounts.length > 0 ) 
      {
        ...
        const displayNames = data?.data?.displayNames ;
        await Promise.all(
          displayNames.map((profileInformation: any) =>
            updateSocialAccountUsername({
              platform: profileInformation?.platform,
  ...
  const onAPIKeySubmit = async (apiKey : string ) => {
    setLoader(true); 
    console.log("api key = " , apiKey) ; 
    ...
    try{
      ...
      if(data.status == 401) 
      {
        ...
        {
          setError("You have exceeded your API quota for the month."); 
          removeToast() ; 
          return ; 
        }
      const {refId} = data?.data ; 
      if(refId)
      {
        // await updateSocialAccount({api_key : apiKey}) ;
        // // logic for adding api key to profile db 
        await fetchAndUpdateActiveSocialAccounts(apiKey); 
  ...
  useEffect(() => {
   console.log("function called ... ") ; 
    ...
    (async () => {
      let accountsData = await getSocialAccounts();
      accountsData = await getSocialAccounts();
  ...
  const totalFollowers = accounts.reduce(
    (sum, account) => sum + parseInt(account.username) || 0,
    0
  );
  const upcomingPostsCount = posts.filter(
    (post) => new Date(post.scheduled_for) > new Date()
  ).length;
  ...
  const  handleLoadInstructionModal = () => {
  ...
  return (
    <div className="">
      {isOpenInstructionModal && <InstructionsModal isOpen={isOpenInstructionModal } onClose={onClose} onAPIKeySubmit={onAPIKeySubmit }  /> } 
     
      <h1 className="text-2xl sm:text-3xl font-bold text-white mb-6 sm:mb-8">
        Welcome back, {profile?.full_name || "User"}!
      </h1>
      {error && <p className="capitalize bg-red-700 text-gray-200 text-center  mb-3   px-2 py-1.5 sm:px-3 sm:py-2 rounded-md text-xs sm:text-sm">{error}</p>}

      {success && <p className=" capitalize bg-green-600 text-center  mb-3   text-white px-2 py-1.5 sm:px-3 sm:py-2 rounded-md text-xs sm:text-sm ">{success}</p>}
      ...
      <div className="bg-gray-800 rounded-xl p-4 sm:p-6 mb-6 sm:mb-8">
        ...
        <p className="text-gray-400 mb-4 sm:mb-6 text-sm sm:text-base">
          Get started by connecting your social media accounts to enable
          automatic posting and analytics.
        </p>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-4 mb-4">
          ...
          <button onClick={handleLoadInstructionModal} className="flex items-center justify-center gap-2 py-2 px-4 bg-gradient-to-r from-purple-500 to-purple-700 text-white font-semibold rounded-lg shadow-md hover:from-purple-600 hover:to-purple-800 transition">
            ...
            >
              <path
                strokeLinecap="round"
                d="M13 16v1a4 4 0 11-8 0v-1a3 3 0 003-3v-4a3 3 0 016 0v4a3 3 0 003 3z"
              />
              <path strokeLinecap="round" d="M22 12v1a4 4 0 01-8 0v-1" />
            </svg>
            <span>
              Connect Social Accounts&nbsp;
              <span className="font-normal text-xs text-purple-200 lg:inline hidden">
                (via Ayrshare)
              </span>
            </span>
          </button>
          ...
          {accounts?.length >= 1 &&
            accounts.find((account) => account?.platform === "twitter") && (
              <button
                ...
                className={`flex items-center justify-center p-2 sm:p-3 rounded-xl transition-colors ${
                  platform == "twitter"
              ...
              <button
                ...
                className={`flex items-center justify-center p-2 sm:p-3 rounded-xl transition-colors ${
                  platform == "instagram"
                    ...
                    : "bg-gray-200 text-purple-500 hover:bg-gray-300"
                }`}
              >
        ...
        <div className="space-y-3 sm:space-y-4">
          {posts
            .filter(
              (post) =>
                post?.status === "pending" &&
                parseISO(post?.scheduled_for) >= new Date()
            )
            .slice(0, 3)
            .map((post) => (
              <div
              ...
              >
                ...
                <div className="text-left sm:text-right flex-shrink-0">
                  <p className="text-purple-400 text-xs sm:text-sm">
                    {`${parseISO(post?.scheduled_for).getUTCDate()}/${
                      parseISO(post?.scheduled_for).getUTCMonth() + 1
                    }/${parseISO(post?.scheduled_for).getFullYear()}`}
                  </p>
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-4 sm:p-6">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        {trend && (
          <span className="text-green-500 text-xs sm:text-sm">{trend}</span>
        )}
      </div>
